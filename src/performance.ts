...
const setResults = (opts: Types.Options, arr: any[]) => {
  return {
    timings: {
      ...
      min: Stats.min(arr),
      max: Stats.max(arr),
      perc50: Stats.perc(arr, 0.5),
      perc75: Stats.perc(arr, 0.75),
      perc90: Stats.perc(arr, 0.9),
    },
...
export default class LoadTest {
  ...
  private async setResults(responses: any) {
    ...
    for (const response of responses) {
      let resp = response;
      if(this.options.executor === "iterations-per-second" || this.options.executor === "variable-rate") {
        resp = response.value;
      }
      metrics.push({
